...
export const DynamicDirectionTitle = ({ direction }: { direction: string }) => {
  ...
  return (
    <>
      ...
      <Link href={PAGES_PATHS.DIRECTIONS()}>Наши направления</Link>{" "}
      <span>{title}</span>
    </>
  );
};
...
export const DynamicDocumentTitle = ({ slug }: { slug: string }) => {
  const queryDocument = useGetContactDetail(slug);
  const title = queryDocument.document?.title;

  if (queryDocument.isLoading)
    return (
      <>
        {Array.from({ length: 3 }).map((_, index) => (
          <Skeleton
...
export const PATHS_BREADCRUMB: IBreadcrumbItem[] = [
  ...
  {
    path: PAGES_PATHS.DOCUMENTS(":slug"),
    title: (params: { slug: string }) => (
      <DynamicDocumentTitle slug={params.slug} />
    ),
  },
