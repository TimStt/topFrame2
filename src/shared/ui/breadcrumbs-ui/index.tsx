...
export const DynamicDirectionTitle = ({ direction }: { direction: string }) => {
  ...
  return (
    <>
      ...
      <Link href={PAGES_PATHS.DIRECTIONS(':direction')}>Наши направления</Link> <span>{title}</span>
    </>
  )
}
...
export const DynamicProductTitle = ({ slug }: { slug: string }) => {
  ...
  if (queryVacancy.isLoading)
    return (
      <>
        {Array.from({ length: 3 }).map((_, index) => (
          <Skeleton key={index} width={'60px'} height={'15px'} style={{ margin: '0 10px 0 0' }} />
        ))}
      </>
    )
  return (
...
export const PATHS_BREADCRUMB: IBreadcrumbItem[] = [
...

  {
    ...
    title: (params: { slug: string }) => <DynamicProductTitle slug={params.slug} />,
  },
...

  {
    ...
    title: (
      <>
        {' '}
        <Link href={PAGES_PATHS.HOME}>Главная</Link> <span>Политика конфиденциальности</span>
      </>
    ),
  },
...

  {
    ...
    title: (params: { slug: string }) => <DynamicDirectionTitle direction={params.slug} />,
  },
]
